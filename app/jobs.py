...
from app.lrc import LrcParseResult, parse_lrc
...
class JobStore:
    ...
    async def create_job(
        ...
        job = Job(id=job_id, output_format=output_format, tts_options=tts_options.__dict__, tts_url=tts_url)
        self.jobs[job_id] = job
        task = asyncio.create_task(
            self._run_job(job, job_dir, original_path, lrc_path, reference_path, tts_options, tts_url),
    ...
    async def _run_job(
        ...
        try:
            ...
            lrc_text = lrc_path.read_text(encoding="utf-8-sig")
            parsed = parse_lrc(lrc_text, audio_duration_ms=audio_info.duration_ms)
            ...
            if not parsed.lines:
                raise RuntimeError("LRC 中没有可处理的有效字幕行。")
